...
CREATE TABLE
    `ox_mdt_reports_criminals` (
        ...
        `stateid` VARCHAR(50) COLLATE utf8_general_ci NOT NULL,
...
CREATE TABLE
    IF NOT EXISTS `ox_mdt_announcements` (
        ...
        `creator` varchar(50) COLLATE utf8_general_ci NOT NULL,
...
ALTER TABLE players
ADD COLUMN firstname VARCHAR(255) GENERATED ALWAYS AS (JSON_UNQUOTE(JSON_EXTRACT(charinfo, '$.firstname'))) STORED,
ADD COLUMN lastname VARCHAR(255) GENERATED ALWAYS AS (JSON_UNQUOTE(JSON_EXTRACT(charinfo, '$.lastname'))) STORED;
